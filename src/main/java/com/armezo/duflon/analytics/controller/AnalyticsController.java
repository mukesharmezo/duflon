...
//		}
		for (Iterator<ParticipantRegistration> iterator = partList.iterator(); iterator.hasNext();) {
			...
			if ((p.getStatus() != null && p.getStatus().equalsIgnoreCase("H")) || (p.getRegStatus()!=null && p.getRegStatus().equals("1"))) {
				Optional<AnalyticsAll> ana = allService.getAnalyticsByAccesskey(p.getAccessKey(), p.getHreId(),p.getId());
				if (ana.isPresent()) {
		...
		if (!partList.isEmpty()) {
			for (ParticipantRegistration part : partList) {
				String exp = "",  interviewStatus = "", age="";
				...
				if (part.getExperience() != null) {
					exp = part.getExperience();
				}
				analytics.setCandidateExperience(exp);
				...
				Optional<InterviewScore> iScore = interviewService.findByAccesskeyAndInterviewCount(part.getAccessKey(),2);
				...
				if (part.getHiredStatus() != null && part.getHiredDate().equals("Y")) {
	...
	public String showAllAnalytics(Model model, HttpSession session) {
		if (session.getAttribute("role") != null) {
			...
			if (role.equalsIgnoreCase("HRE")) {
				...
				hres.add(dl.get());
				hreIdList.add(hreId);
			} else if (role.equalsIgnoreCase("LM") || role.equalsIgnoreCase("SA")) {
				List<HRE> hres2 = hreService.getAllDeealer();
				for (HRE hre : hres2) {
	...
	public String searchDashboard(@RequestParam("outletCode") String outletCode,
		...
		if (session.getAttribute("userId") != null) {
			...
			if (role.equalsIgnoreCase("HRE")) {
				...
				Optional<HRE> dl = hreService.getById(dId);
				...
				List<HRE> deal = hreService.getAllDeealer();
...
*/
	...
	public String getStateByRegion(@RequestParam String regionCode, HttpSession session) {
		if (session.getAttribute("userId") != null) {
			...
			if (role.equalsIgnoreCase("HRE")) {
				Long hreId = Long.parseLong(session.getAttribute("userId").toString());
				outletList = outletService.getOutletByhreId(hreId);
	...
	public String getDealersByCityCode(@RequestParam String cityCode, HttpSession session) {
		if (session.getAttribute("userId") != null) {
			...
			for (Outlet d : outlets3) {
				dealershipOption = String.valueOf(dealershipOption) + "<option value='" + d.getOutletName() + "'>"
		...
		if (session.getAttribute("userId") != null) {
			...
			if (role.equalsIgnoreCase("HRE")) {
				...
				if (outletCode != null && outletCode != "") {
					//Optional<Outlet> out = outletService.getOutletByOutletCodeAndhreId(outletCode, dId);
					//if (out.isPresent()) {
					//	outlets.add(out.get());
					//}
					...
					outlets.addAll(outletService.findByhreId(dId));
				}
			} else if (role.equalsIgnoreCase("LM")) {
				if (outletCode != null && outletCode != "") {
					//Optional<Outlet> out = outletService.getOutletByOutletCode(outletCode);
					List<Outlet> out = outletService.getByOutletName(outletCode);
					for (Outlet o: out) {
						outlets.add(o);
					}
				} else {
	...
	public String getFSDMByRegionCode(@RequestParam String regionCode, HttpSession session) {
		if (session.getAttribute("userId") != null) {
			...
			List<LineManager> fsdms = new ArrayList<LineManager>();
			if(regionCode!=null && regionCode!="") {
			...
			List<LineManager> fsdms2 = fsdms.stream().sorted((f1,f2)-> f1.getName().compareToIgnoreCase(f2.getName())).collect(Collectors.toList());
			for(FSDM f : fsdms2) {
	...
	public String getFSDMByParentDealer(@RequestParam String pdCode) {
		String fsdmOption = "<option value=''>FSDM</option>";
		Set<LineManager> fsdms = new LinkedHashSet<LineManager>();
		List<Outlet> outlets = new ArrayList<Outlet>();
	...
	public String getDealerByFSDMId(@RequestParam String fsdmId ) {
		...
		Integer fId = null;
		Set<Outlet> outlets = new LinkedHashSet<Outlet>();
		Set<HRE> dealers = new LinkedHashSet<HRE>();
		...
		if(fsdmId!=null && fsdmId!="") {
			...
			List<HRE> dealers2 = dealers.stream().sorted((d1,d2)->d1.getName().compareToIgnoreCase(d2.getName())).collect(Collectors.toList());
