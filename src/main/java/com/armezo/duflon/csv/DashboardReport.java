...
/*
	@Autowired
	private HREService dealerService;
	@Autowired
	...
	public String showLinksForCSV(@RequestParam String flag, HttpSession session, Model model) {
		if (session.getAttribute("role") != null) {
			...
			if (role.equalsIgnoreCase("HRE")) {
				...
				outlets.addAll(outletService.getOutletByhreId(hreId));
				hreIdList.add(hreId);
			...
			} else if (role.equalsIgnoreCase("HO")  || role.equalsIgnoreCase("SA")) {
				List<HRE> deal = dealerService.getAllDeealer();
			...
			CandidateNonAutoExperiencePayload nonAuto = allService.getCandidateNonAutoExperience(hreIdList,dateFrom,dateTo);
			...
			// Search Filter Data
		/*	List<HRE> dealers2 = dealers.stream().sorted((d1, d2) -> d1.getName().compareToIgnoreCase(d2.getName()))
			...
			List<Outlet> outlets3 = new ArrayList<>();
			String temp="";
			for(Outlet o:outlets2) {
				if(temp.equals(o.getOutletName())) {
					continue;
				}else {
					temp = o.getOutletName();
					outlets3.add(o);
				}
			}
			...
			model.addAttribute("dealerShipList", outlets3);
	...
	public String getDashBoardReportFilterData(@RequestParam("outletCode") String outletCode,
			@RequestParam("dealershipName") String dealershipName, @RequestParam("regionCode") String regionCode,
			@RequestParam("stateCode") String stateCode, @RequestParam("cityCode") String cityCode, @RequestParam(required = false) String approved,
		...
		if (session.getAttribute("role") != null) {
			...
			if(approved==null || !approved.equals("2")) {
				approved="";
			}
			...
			List<HRE> dealers = new ArrayList<>();
			...
			if (role.equalsIgnoreCase("HRE")) {
				...
				Optional<HRE> dl = dealerService.getById(dId);
				List<Outlet> ot = outletService.findByhreId(dId);
			...
			if(dealershipName!=null && dealershipName!="") {
				...
				List<Outlet> outlets6 = new ArrayList<>();
				for(Long id : hreIdList) {
			...
			OverviewPayload overview = allService.getOverviewAnalytics(outletCode, searchOutlets, regionCode,
					stateCode, cityCode, approved, dateFrom, dateTo, hreIdList);
			ActionPointsPayload action = allService.getActionPending(outletCode, searchOutlets, regionCode, stateCode,
					cityCode, approved, dateFrom, dateTo, hreIdList);
			RecruitmentSourcePayload source = allService.getRecruitmentSourceAnalytics(outletCode, searchOutlets,
					regionCode, stateCode, cityCode, approved, dateFrom, dateTo, hreIdList);
			CandidateExperiencePayload exp = allService.getCandidateExpAnalytics(outletCode, searchOutlets, regionCode,
					stateCode, cityCode, approved, dateFrom, dateTo, hreIdList);
			CandidateNonAutoExperiencePayload nonAuto = allService.getCandidateNonAutoExperience(outletCode,
					searchOutlets, regionCode, stateCode, cityCode, approved, dateFrom, dateTo, hreIdList);
			AssessmentReportPayload assessment = allService.getAssessmentReportAnalytics(outletCode, searchOutlets,
					regionCode, stateCode, cityCode, approved, dateFrom, dateTo, hreIdList);
			DesignationTypePayload designation = allService.getDesignationTypeAnalytics(outletCode, searchOutlets,
					regionCode, stateCode, cityCode, approved, dateFrom, dateTo, hreIdList);
			AgeWisePayload age = allService.getAgeWiseAnalytics(outletCode, searchOutlets, regionCode, stateCode,
					cityCode, approved, dateFrom, dateTo, hreIdList);
			GenderDiversityPayload gender = allService.getGenderAnalytics(outletCode, searchOutlets, regionCode,
					stateCode, cityCode, approved, dateFrom, dateTo, hreIdList);
			//Get All Analytics
			List<AnalyticsAll> analyticsAlls = allService.getDataByAnyTypeLoginByhreIds(outletCode,searchOutlets,regionCode,stateCode,cityCode, approved, hreIdList,dateFrom,dateTo);
			//Only Aggregate Data
			List<ModelDashboardAggregate> aggregates = getAggregateData(analyticsAlls);
			...
			if(approved!=null && approved.equals("2")) {
			filterPayload.setApproved(2);
			}else {
				filterPayload.setApproved(0);
			}
			List<HRE> dealers2 = dealers.stream().sorted((d1, d2) -> d1.getName().compareToIgnoreCase(d2.getName()))
					.collect(Collectors.toList());
			List<Region> regions2 = regions.stream()
	...
	private List<ModelDashboardAggregate> getAggregateData(List<AnalyticsAll> analyticsAlls) {
		...
		for(AnalyticsAll analytics : analyticsAlls) {
			...
			aggregate.setRegion(analytics.getRegion());
			aggregate.setDealerCode(analytics.getDealerCode());
			...
			aggregate.setDealership(analytics.getDealershipName());
	...
	public List<ModelParticpantView> getParticipantAllDataByAccesskeyList(List<String> accesskeyList, String status) {
		...
		for (AnalyticsAll a : list) {
			...
			//Setting Outlet Related Data
			mpv.setRegionCode(a.getRegion());
			mpv.setDealerCode(a.getDealerCode());
			//get city name by city code
			...
			mpv.setDealerName(a.getDealershipName());
	...
	public ResponseEntity<?> generateDashboardReport(@RequestParam("outletCode") String outletCode,
		...
		if (session.getAttribute("userId") != null) {
			List<Long> hreIdList = new ArrayList<>();
			...
			if (role.equalsIgnoreCase("HRE")) {
				Long hreId = Long.parseLong(session.getAttribute("userId").toString());
				List<Outlet> out = outletService.getOutletByhreId(hreId);
				outlets.addAll(out);
				...
				List<HRE> dealers = dealerService.getAllDeealer();
				regions.addAll(regionService.getAllRegion());
			...
			//Below var for Counting all regions data in one row as total
			int registeredCount=0,assessmentCount=0,passedCount=0,offeredCount=0,recruitedCount=0,assessmentActionCount=0,interviewCount=0,docCount=0,praarambhCount=0,fsdmApprovalCount=0,
					refferalCount=0,walkinCount=0,advertisementCount=0,jobCount=0,socialCount=0,otherCount=0,salesCount=0,nonSalesCount=0,maleCount=0,femaleCount=0,auto3Count=0,auto3_6Count=0,auto6_12Count=0,auto1_2Count=0,
					auto2_5Count=0,auto5_10Count=0,auto10MoreCount=0,non3Count=0,non3_6Count=0,non6_12Count=0,non1_2Count=0,non2_5Count=0,non5_10Count=0,non10MoreCount=0,age18_20Count=0,age20_25Count=0,
					age25_30Count=0,age30_35Count=0,age35_40Count=0,age40MoreCount=0,assess40Count=0,assess40_60Count=0,assess60_80Count=0,assess80MoreCount=0;
			int count = 2;
			...
			for (Region r : list) {
				...
				OverviewPayload overview = allService.getOverviewAnalytics(outletCode, searchOutlets, regionCode2,
						stateCode, cityCode, approved, dateFrom, dateTo, hreIdList);
				ActionPointsPayload action = allService.getActionPending(outletCode, searchOutlets, regionCode2,
						stateCode, cityCode, approved, dateFrom, dateTo, hreIdList);
				RecruitmentSourcePayload source = allService.getRecruitmentSourceAnalytics(outletCode, searchOutlets,
						regionCode2, stateCode, cityCode, approved, dateFrom, dateTo, hreIdList);
				CandidateExperiencePayload exp = allService.getCandidateExpAnalytics(outletCode, searchOutlets,
						regionCode2, stateCode, cityCode, approved, dateFrom, dateTo, hreIdList);
				CandidateNonAutoExperiencePayload nonAuto = allService.getCandidateNonAutoExperience(outletCode,
						searchOutlets, regionCode2, stateCode, cityCode, approved, dateFrom, dateTo, hreIdList);
				AssessmentReportPayload assessment = allService.getAssessmentReportAnalytics(outletCode, searchOutlets,
						regionCode2, stateCode, cityCode, approved, dateFrom, dateTo, hreIdList);
				DesignationTypePayload designation = allService.getDesignationTypeAnalytics(outletCode, searchOutlets,
						regionCode2, stateCode, cityCode, approved, dateFrom, dateTo, hreIdList);
				AgeWisePayload age = allService.getAgeWiseAnalytics(outletCode, searchOutlets, regionCode2, stateCode,
						cityCode, approved, dateFrom, dateTo, hreIdList);
				GenderDiversityPayload gender = allService.getGenderAnalytics(outletCode, searchOutlets, regionCode2,
						stateCode, cityCode, approved, dateFrom, dateTo, hreIdList);
				...
				if(role.equalsIgnoreCase("DL")) {
					for(Outlet o :outlets) {
						cell.setCellValue(o.getOutletName());
					}
				}else {
				...
				registeredCount=registeredCount+overview.getRegistered().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(2);
				...
				assessmentCount=assessmentCount+overview.getAssessments().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(3);
				...
				passedCount=passedCount+overview.getPass().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(4);
				...
				offeredCount=offeredCount+overview.getOffer().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(5);
				...
				interviewCount=interviewCount+action.getInterviewStatus().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(8);
				...
				docCount=docCount+action.getDocumentUploadStatus().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(9);
				...
				praarambhCount=praarambhCount+action.getPrarambhStatus().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(10);
				...
				fsdmApprovalCount=fsdmApprovalCount+action.getFsdmApproval().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(11);
				...
				refferalCount=refferalCount+source.getReferrals().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(12);
				...
				walkinCount=walkinCount+source.getDirectWalkIn().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(13);
				...
				advertisementCount=advertisementCount+source.getAdvertisement().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(14);
				...
				jobCount=jobCount+source.getJobConsultant().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(15);
				...
				socialCount=socialCount+source.getSocialMedia().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(16);
				...
				otherCount=otherCount+source.getOthers().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(17);
				...
				salesCount=salesCount+designation.getSales().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(18);
				...
				nonSalesCount=nonSalesCount+designation.getNonSales().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(19);
				...
				maleCount=maleCount+gender.getMale().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(20);
				...
				femaleCount=femaleCount+gender.getFemale().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(21);
				...
				auto3Count += exp.getLessThan3Months().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(22);
				...
				auto3_6Count +=exp.getMonths3To6().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(23);
				...
				auto6_12Count +=exp.getMonths6To1Year().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(24);
				...
				auto1_2Count += exp.getYear1To2Year().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(25);
				...
				auto2_5Count += exp.getYear2To5Year().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(26);
				...
				auto5_10Count += exp.getYear5To10Year().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(27);
				...
				auto10MoreCount += exp.getMoreThan10Year().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(28);
				...
				non3Count += nonAuto.getLessThan3Months().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(29);
				...
				non3_6Count += nonAuto.getMonths3To6().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(30);
				...
				non6_12Count += nonAuto.getMonths6To1Year().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(31);
				...
				non1_2Count += nonAuto.getYear1To2Year().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(32);
				...
				non2_5Count += nonAuto.getYear2To5Year().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(33);
				...
				non5_10Count += nonAuto.getYear5To10Year().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(34);
				...
				non10MoreCount += nonAuto.getMoreThan10Year().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(35);
				...
				age18_20Count += age.getLessThan20().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(36);
				...
				age20_25Count += age.getBetween20To25().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(37);
				...
				age25_30Count += age.getBetween25To30().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(38);
				...
				age30_35Count += age.getBetween30To35().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(39);
				...
				age35_40Count += age.getBetween35To40().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(40);
				...
				age40MoreCount += age.getMoreThan40().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(41);
				...
				assess40Count += assessment.getLessThan40().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(42);
				...
				assess40_60Count += assessment.getBetween40To60().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(43);
				...
				assess60_80Count += assessment.getBetween60To80().size();
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(44);
				...
				assess80MoreCount += assessment.getMoreThan80().size();
				}else {
					cell.setCellValue("");
				}
				count++;
			}
			//Setting row for total count
			if(role.equalsIgnoreCase("HO")  || role.equalsIgnoreCase("SA")) {
				...
				if(registeredCount>0) {
					cell.setCellValue(registeredCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(2);
				if(assessmentCount>0) {
					cell.setCellValue(assessmentCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(3);
				if(passedCount>0) {
					cell.setCellValue(passedCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(4);
				if(offeredCount>0) {
					cell.setCellValue(offeredCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(5);
				if(recruitedCount>0) {
					cell.setCellValue(recruitedCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(6);
				if(assessmentActionCount>0) {
					cell.setCellValue(assessmentActionCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(7);
				if(interviewCount>0) {
					cell.setCellValue(interviewCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(8);
				if(docCount>0) {
					cell.setCellValue(docCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(9);
				if(praarambhCount>0) {
					cell.setCellValue(praarambhCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(10);
				if(fsdmApprovalCount>0) {
					cell.setCellValue(fsdmApprovalCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(11);
				if(refferalCount>0) {
					cell.setCellValue(refferalCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(12);
				if(walkinCount>0) {
					cell.setCellValue(walkinCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(13);
				if(advertisementCount>0) {
					cell.setCellValue(advertisementCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(14);
				if(jobCount>0) {
					cell.setCellValue(jobCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(15);
				if(socialCount>0) {
					cell.setCellValue(socialCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(16);
				...
				cell = row.createCell(17);
				if(salesCount>0) {
					cell.setCellValue(salesCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(18);
				if(nonSalesCount>0) {
					cell.setCellValue(nonSalesCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(19);
				if(maleCount>0) {
					cell.setCellValue(maleCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(20);
				if(femaleCount>0) {
					cell.setCellValue(femaleCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(21);
				if(auto3Count>0) {
					cell.setCellValue(auto3Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(22);
				if(auto3_6Count>0) {
					cell.setCellValue(auto3_6Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(23);
				if(auto6_12Count>0) {
					cell.setCellValue(auto6_12Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(24);
				if(auto1_2Count>0) {
					cell.setCellValue(auto1_2Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(25);
				if(auto2_5Count>0) {
					cell.setCellValue(auto2_5Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(26);
				if(auto5_10Count>0) {
					cell.setCellValue(auto5_10Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(27);
				if(auto10MoreCount>0) {
					cell.setCellValue(auto10MoreCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(28);
				if(non3Count>0) {
					cell.setCellValue(non3Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(29);
				if(non3_6Count>0) {
					cell.setCellValue(non3_6Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(30);
				if(non6_12Count>0) {
					cell.setCellValue(non6_12Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(31);
				if(non1_2Count>0) {
					cell.setCellValue(non1_2Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(32);
				if(non2_5Count>0) {
					cell.setCellValue(non2_5Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(33);
				if(non5_10Count>0) {
					cell.setCellValue(non5_10Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(34);
				if(non10MoreCount>0) {
					cell.setCellValue(non10MoreCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(35);
				if(age18_20Count>0) {
					cell.setCellValue(age18_20Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(36);
				if(age20_25Count>0) {
					cell.setCellValue(age20_25Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(37);
				if(age25_30Count>0) {
					cell.setCellValue(age25_30Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(38);
				if(age30_35Count>0) {
					cell.setCellValue(age30_35Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(39);
				if(age35_40Count>0) {
					cell.setCellValue(age35_40Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(40);
				if(age40MoreCount>0) {
					cell.setCellValue(age40MoreCount);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(41);
				if(assess40Count>0) {
					cell.setCellValue(assess40Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(42);
				if(assess40_60Count>0) {
					cell.setCellValue(assess40_60Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(43);
				if(assess60_80Count>0) {
					cell.setCellValue(assess60_80Count);
				}else {
					cell.setCellValue("");
				}
				cell = row.createCell(44);
				if(assess80MoreCount>0) {
					cell.setCellValue(assess80MoreCount);
				}else {
					cell.setCellValue("");
				}
			}
			
			String fileName = DataProccessor.getReportName("Dashboard Report");
