...
if(session.getAttribute("success_msg") != null){
	msg = session.getAttribute("success_msg").toString().trim();
}
 ...
 .form {
  ...
  .form__button--submit img{margin-right: 10px;}
    ...
    function updateMSPIN(){
		...
		document.forms[0].action="deactivateDealerMSPINPro";
    ...
    <div >
        <div class="form" style="max-width: 600px!important;">
            <form class="form__section"   id="testForm">
                <h1 style="font-size:25px !important;">Deactivate Dealer</h1>
                <div class="form__block">
                    <input type="text" name="mspin" id="upeate_mspin" placeholder="Enter mspin">
                </div>
                 <div class="error-chage" id="error_success"></div>
                <div class="form__block">
                    <input type="button" class="form__button form__button--submit" onclick="updateMSPIN()" value="Proceed">
                </div>
            </form>
        </div>
    </div>
...
$('.error-chage').append('MSPIN Deactivated.');
  $('.error-chage').show();   
<% }%>
<%if(msg.equals("2")){%>
$('.error-chage').append('MSPIN activated.');
  $('.error-chage').show();   
<% }%>
<%if(msg.equals("3")){%>
$('.error-chage').append('MSPIN not persent.');
  $('.error-chage').show();   
<% }%>
</script>
</html>
